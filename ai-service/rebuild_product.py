...
def rebuild_product(product_id: str):
    ...
    try:
        ...
        if not product:
            print(f"⚠️ Produk dengan ID {product_id} tidak ditemukan.")
            return

        print(f"\n🚀 Memulai rebuild untuk produk {product_id}...")
        print(f"📅 Mulai pada: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")

        # 1. Update keyword summary
        update_keyword_summary(product_id)

        # 2. Generate dan simpan insight
        result = generate_insight_for_product(product_id, db)

        if result["status"] == "ok":
            print(f"\n✅ Rebuild selesai untuk produk {product_id}")
        else:
            print(f"\n❌ Rebuild gagal untuk produk {product_id}: {result.get('error')}")
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        print("Usage: python rebuild_product.py <product_id>")
        sys.exit(1)
    product_id = sys.argv[1]
    rebuild_product(product_id)